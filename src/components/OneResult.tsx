...
const OneResult = ({ occupation }: OneResultProps) => {
  ...
  const getReleventCompetencies = (id: string) => {
    ...
    getCompetenciesByOccupationId(data)
      .then((response: AxiosResponse<ICompetencies>) => {
        ...
        const shortCompetenciesList =
          fullCompetenciesList.metadata.enriched_candidates_term_frequency.competencies.slice(
            0,
            10
          );
  ...
  return (
    <>
      <li
        onClick={() => {
          getReleventCompetencies(occupation.concept_taxonomy_id);
          toggleChart();
        }}
        className="position-relative"
      >
        <div className="oneResult-closed">
          <h3 className="competencies-header">
            {occupation.occupation_label} {loading && <DigiLoaderSpinner />}
          </h3>
          <button
            aria-label="Toggla statistikvy för detta yrke."
            className="open-btn"
          >
            {showChart ? <DigiIconArrowUp /> : <DigiIconArrowDown />}
          </button>
        </div>
        {competencies && showChart && (
          <h4 className="competencies-headline">Efterfrågade kompetenser:</h4>
        )}
